...
class BooksToScrapeSpider(scrapy.Spider):
    ...
    def parse_category_books(self, response):
        ...
        for books in response.css('article.product_pod'):
            items['book_title'] = books.css('h3 > a::attr(title)').extract_first()
            items['book_price'] = books.css('p.price_color::text').extract_first()
            items['book_image_URL'] = response.urljoin(books.css('div.image_container > a > img::attr(src)').extract_first())
            items['book_details_page_URL'] = response.urljoin(books.css('div.image_container > a::attr(href)').extract_first())
            if items['book_title'] == 'Frankenstein':
                yield items
            # yield {
